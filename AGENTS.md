...
- `disable-model-invocation` is banned in every SKILL.md (third ban, 2026-08-20; check-repo fails on presence). On current Claude Code the flag removes the skill from the model's own skill list, and a typed `/name` is executed by the model calling the Skill tool, so a flagged skill cannot be invoked at all until `/reload-plugins`. Picker visibility and headless dispatch do not prove invocability: measure with the model's own available-skills list (a fresh interactive session, no reload). User-invoked intent lives in description wording plus per-machine consumer `skillOverrides: name-only` (never `user-invocable-only`, which hides the skill the same way). Descriptions say what the skill does, when to reach for it, and the nearest exclusion; never activation bait. Reasoning and measurements: `git show b4441a3` (0.7.5) and `git show accdcba` (0.21.1).
