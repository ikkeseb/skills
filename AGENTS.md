...
Each skill lives in `skills/<name>/` with a `SKILL.md` whose YAML frontmatter (`name`, `description`, required `disable-model-invocation: true`, optional `allowed-tools`) is how Claude Code discovers the skill. Other files in the skill folder (references, scripts, assets) load on demand: keep the core workflow in `SKILL.md`, put branch-only or bulky reference in sibling files, and word each pointer to say when to read it.
...
- Skills are reached by explicit invocation (`/name` in Claude Code, the `$` picker in Codex), with no exceptions, and since 0.20.0 the frontmatter enforces it: every `SKILL.md` carries `disable-model-invocation: true` (check-repo requires it). Descriptions therefore serve the picker and the docs — say what the skill does, when to reach for it, and the nearest exclusion; never activation bait. `handoff` briefly carried a model-invokable carve-out (0.7.4–0.8.1); reverted — don't reintroduce one without a decision that says so.
...
- `disable-model-invocation: true` is required in every skill (the 2026-08-20 canary cleared the old ban: typed `/name` works headless and the skill stays visible in the TUI picker, verified on both maintainer machines for personal/symlink installs; Claude's docs state the flag travels with plugin skills, though an isolated plugin install has not been re-canaried). For symlink deployments, per-machine trigger tuning still belongs in consumer settings (`skillOverrides`); those do not affect plugin-shipped skills, where the frontmatter flag is now the enforcement.
