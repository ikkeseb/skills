...
- Invocation policy is a per-skill decision (2026-08-20, supersedes the 0.20.0 blanket rule). Most skills stay explicit-invocation only (`/name` in Claude Code, the `$` picker in Codex) and carry `disable-model-invocation: true`; a skill whose trigger is a deliverable request the model should recognize on its own (currently `pretty-html`) omits the flag. check-repo validates the flag's shape, not its presence. Descriptions serve the picker and the docs either way: say what the skill does, when to reach for it, and the nearest exclusion; never activation bait. Record the reasoning in the CHANGELOG when a skill's policy changes.
...
- `disable-model-invocation: true` is safe to carry (the 2026-08-20 canary cleared the old ban: typed `/name` works headless and the skill stays visible in the TUI picker, verified on both maintainer machines for personal/symlink installs; Claude's docs state the flag travels with plugin skills, though an isolated plugin install has not been re-canaried). For symlink deployments, per-machine trigger tuning still belongs in consumer settings (`skillOverrides`); those do not affect plugin-shipped skills, where the frontmatter flag is the enforcement.
