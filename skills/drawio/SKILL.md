...
3. **Write** it to `<descriptive-name>.drawio` (lowercase-with-hyphens), in the location the repository's agent contract (AGENTS.md or equivalent) specifies for deliverables when one exists, otherwise the cwd. Done when: the file exists at the reported path.
