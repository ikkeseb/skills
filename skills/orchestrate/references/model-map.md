...
- **Cheap**: Luna handles bounded readers, extraction, mechanical
  ...
  is a low-priority read-only extra, not a Luna replacement: no shell, no
  writes, and too weak to carry verification.
...
| `gemini-3.8-flash-medium` | agy (read-only) | in the id: `-medium`; `-low` for plain extraction, `-high` for a harder read | Low priority, never ahead of Luna by default: an extra third-family opinion on a read, or low-stakes extraction and general-knowledge questions when the user asks for Google models; never counts or inventories (no shell) and never the owed cross-family verifier; findings are candidates, never decisions | `gpt-6-luna` |
