...
2. **Find the hard gate.** Read the target repo's `CLAUDE.md`, `package.json`
   ...
   signal means refuse (see Anti-patterns); **do not fabricate a gate**. Done
   when you hold a named command or signal and know roughly what one run of it
   costs (run it if cheap, else read CI timings).
...
4. **Bake in the stops.** Every suggestion needs a measurable success stop
   ...
   autonomously, so use one only when a human explicitly owns cancellation. Done
