...
If the path doesn't exist or can't be read, say so and stop. If the target yields no claims, say so in one line, without a table.
...
1. **Workspace files**, cited as `file:line`
...
**2. Table**, with emoji as the status column and plain prose in the others:

| | claim | source | note |
|---|---|---|---|
| ✅ | "the API is called X" | `src/api.ts:42` | direct match |
| ❓ | "X is the fastest framework" | (none) | "fastest" undefined, no benchmark to check against |
